...
import { setLocationAuth } from "../../../lib/storage";

export default function OtpScreen() {
  const { slug, phone, scanToken, maxStamps } = useLocalSearchParams<{
    slug: string;
    phone: string;
    scanToken?: string;
    maxStamps?: string;
  ...
  async function submit() {
    ...
    await setLocationAuth(slug, { token: res.data.token, phone });
    router.replace({
      pathname: "/loyalty/[slug]/card",
      params: { slug, scanToken: scanToken ?? "", maxStamps: maxStamps ?? "10" },
    });
  }
